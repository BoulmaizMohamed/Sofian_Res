<x-admin-layout title="Assign Bed {{ $bed->name }}">

<div style="display:flex;align-items:center;gap:1rem;margin-bottom:1.5rem;">
    <a href="{{ route('admin.dashboard', ['date' => $date]) }}" class="btn btn-secondary btn-sm">← Back</a>
    <h1 class="page-title" style="margin:0;">Assign {{ $bed->name }} ({{ $bed->room->name }})</h1>
</div>
...
<div class="card" style="max-width:600px;">
    <form method="POST" action="{{ route('admin.bed-bookings.store') }}">
        @csrf
        ...
        <div style="display:flex;gap:.75rem;margin-top:1.5rem;">
            ...
            <a href="{{ route('admin.dashboard', ['date' => $date]) }}" class="btn btn-secondary">Cancel</a>
        </div>
    </form>
</div>
